# ----- ABA 1: Pilares (EXPANDIDA) -----
    with tabs[0]:
        ...
        fig = px.bar(df,x='Pilar',y='Força',color='Time',barmode='group',text_auto='.1f',
                     ...
                     title="Comparativo de Pilares")
        fig.update_traces(textposition='outside', textfont=dict(size=14, color='white'))
        fig.update_layout(template='plotly_dark',paper_bgcolor='rgba(0,0,0,0)',plot_bgcolor='rgba(0,0,0,0)',
                          transition_duration=800, hovermode='x unified')
        st.plotly_chart(fig,use_container_width=True)
        
        # Tabela de indicadores usados
        st.markdown("**📊 Indicadores que compõem cada pilar:**")
        col_info1, col_info2 = st.columns(2)
        with col_info1:
            st.markdown(f"""
            <div class="info-card">
                <strong>🏠 {nome_casa}</strong><br>
                <strong>MA:</strong> {ma_A:.1f} (momentum recente)<br>
                <strong>FG:</strong> {fg_A:.1f} (ataque/defesa/meio)<br>
                <strong>CPP:</strong> {cpp_A:.1f} (confronto por prateleira)<br>
                <strong>Psic:</strong> {psic_A:.1f} (consistência/moral/pressão)
            </div>
            """, unsafe_allow_html=True)
        with col_info2:
            st.markdown(f"""
            <div class="info-card">
                <strong>✈️ {nome_fora}</strong><br>
                <strong>MA:</strong> {ma_B:.1f}<br>
                <strong>FG:</strong> {fg_B:.1f}<br>
                <strong>CPP:</strong> {cpp_B:.1f}<br>
                <strong>Psic:</strong> {psic_B:.1f}
            </div>
            """, unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

        # Radar de Força Setorial (com animação)
        ...
        fig_radar.add_trace(go.Scatterpolar(r=[atq_B,def_B,mei_B],theta=['Ataque','Defesa','Meio'],
                                            ...
                                            hovertemplate='%{r:.1f}<br>%{theta}'))
        fig_radar.update_layout(polar=dict(radialaxis=dict(range=[0,100], tickfont=dict(color='white')),
                                           angularaxis=dict(tickfont=dict(color='white'))),
                                template='plotly_dark',paper_bgcolor='rgba(0,0,0,0)',
                                transition_duration=800,
                                title=f"Comparação Setorial: {nome_casa} vs {nome_fora}")
        st.plotly_chart(fig_radar,use_container_width=True)
        st.markdown('</div>',unsafe_allow_html=True)

    # ----- ABA 2: Estilo (EXPANDIDA COM ANIMAÇÃO) -----
    with tabs[1]:
        st.markdown('<div class="card-premium"><div class="card-header-premium">🎭 PERFIS TÁTICOS</div>',unsafe_allow_html=True)
        
        # Cards de perfil lado a lado
        col_perf1,col_perf2=st.columns(2)
        with col_perf1:
            cor_perfil = '#F0C040' if 'Dominante' in perfil_A or 'Pressão' in perfil_A else '#4A90D9' if 'Reativo' in perfil_A else '#B0B8C0'
            st.markdown(f"""<div style="background:rgba(240,192,64,0.05);border:1px solid {cor_perfil};border-radius:12px;padding:20px;text-align:center;">
                <div style="font-size:18px;font-weight:700;color:#F0C040;margin-bottom:8px;">🏠 {nome_casa}</div>
                <div style="font-size:28px;font-weight:900;color:{cor_perfil};">{perfil_A}</div>
        ...
        with col_perf2:
            ...
            st.markdown(f"""<div style="background:rgba(74,144,217,0.05);border:1px solid {cor_perfil};border-radius:12px;padding:20px;text-align:center;">
                <div style="font-size:18px;font-weight:700;color:#4A90D9;margin-bottom:8px;">✈️ {nome_fora}</div>
                <div style="font-size:28px;font-weight:900;color:{cor_perfil};">{perfil_B}</div>
                <div style="font-size:13px;color:#B0B8C0;margin-top:8px;">Dominância: {estilo_B:.1f}/100</div>
            </div>""",unsafe_allow_html=True)
        
        # Gráfico de barras dos indicadores de estilo (ANIMADO)
        st.markdown("**📊 Indicadores de Estilo de Jogo**")
        st.markdown("*Quanto maior a barra, mais presente é o indicador no estilo do time.*")
        
        indicadores_estilo = ['Posse', 'Chutes', 'Cruzamentos', 'Bolas Enfiadas', 'Passes Curtos', 'Presença Ofensiva']
        # Usar dados disponíveis ou padrão
        vals_estilo_A = [
            posse_casa, fa_casa, dados.get('crs_casa', eca_casa), 
            dados.get('thrball_casa', 0.5), dados.get('shortpass_casa', 300), 
            dados.get('attthird_casa', 30)
        ]
        vals_estilo_B = [
            posse_fora, fa_fora, dados.get('crs_fora', eca_fora), 
            dados.get('thrball_fora', 0.5), dados.get('shortpass_fora', 300), 
            dados.get('attthird_fora', 30)
        ]
        ...
        df_estilo = pd.DataFrame({
            'Indicador': indicadores_estilo * 2,
            'Time': [nome_casa]*6 + [nome_fora]*6,
            'Valor': vals_A_norm + vals_B_norm
        })
        
        fig_estilo = px.bar(df_estilo, x='Indicador', y='Valor', color='Time', barmode='group',
                            color_discrete_map={nome_casa:'#F0C040',nome_fora:'#4a90d9'},
                            title="Comparativo de Estilo de Jogo")
        fig_estilo.update_traces(textposition='outside')
        fig_estilo.update_layout(template='plotly_dark',paper_bgcolor='rgba(0,0,0,0)',
                                 transition_duration=800)
        st.plotly_chart(fig_estilo,use_container_width=True)
        
        # Legenda de perfis
        st.markdown("""<div class="info-card" style="margin-top:16px;">
            <strong>🏆 Dominante:</strong> Controla posse, finaliza muito, pressiona.<br>
            <strong>🔥 Pressão Alta:</strong> Extremamente agressivo.<br>
            <strong>⚡ Reativo/Contra‑ataque:</strong> Pouca posse, transições rápidas.<br>
            <strong>🛡️ Defensivo:</strong> Prioriza não sofrer gols.<br>
            <strong>⚖️ Equilibrado:</strong> Sem extremos.<br>
            <strong>🅿️ Jogo Pelas Pontas:</strong> Muitos cruzamentos.<br>
            <strong>⬆️ Jogo Vertical:</strong> Bolas enfiadas, profundidade.
        </div>""",unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)
    ...
    with tabs[2]:
        ...
        stats_completas = [
            ...
            ('Finalizações Sofridas', fas_casa, fas_fora, 'menor', '🛡️'),
            ('Faltas Cometidas', fc_casa, fc_fora, 'menor', '🟨'),
            ('Cartões Amarelos', ca_casa, ca_fora, 'menor', '🟨'),
            ('Desarmes', des_casa, des_fora, 'maior', '💪'),
        ...
        for nome, vA, vB, tipo, icone in stats_completas:
            if tipo == 'maior':
                ...
                cor_A = '#00E676' if vA > vB else '#E0E0E0'
                cor_B = '#00E676' if vB > vA else '#E0E0E0'
            else:
                vant = nome_casa if vA < vB else nome_fora if vB < vA else "Empate"
                cor_A = '#00E676' if vA < vB else '#E0E0E0'
                cor_B = '#00E676' if vB < vA else '#E0E0E0'
            ...
            st.markdown(f"""
            <div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.03);font-size:14px;color:#E0E0E0;">
                <span>{icone} {nome}</span>
                <span style="color:{cor_A};font-weight:600;">{nome_casa}: {vA:.1f}</span>
                <span style="color:{cor_B};font-weight:600;">{nome_fora}: {vB:.1f}</span>
                <span style="font-weight:700;color:#F0C040;">➡️ {vant}</span>
            </div>
            """,unsafe_allow_html=True)
        
        # Resumo de quem domina mais estatísticas
        vant_A = sum(1 for nome,vA,vB,tipo,_ in stats_completas if (tipo=='maior' and vA>vB) or (tipo=='menor' and vA<vB))
        vant_B = sum(1 for nome,vA,vB,tipo,_ in stats_completas if (tipo=='maior' and vB>vA) or (tipo=='menor' and vB<vA))
        
        st.markdown(f"""
        <div class="info-card" style="margin-top:16px;text-align:center;">
            <strong>{nome_casa}</strong> vence em <strong>{vant_A}/9</strong> estatísticas | 
            <strong>{nome_fora}</strong> vence em <strong>{vant_B}/9</strong> estatísticas
        </div>
        """,unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)
        # ----- ABA 4: Heatmap (ANIMADO COM MOVIMENTO) -----
    with tabs[3]:
        ...
        # Heatmap animado com partículas de movimento
        ...
        frames = []
        for t in range(10):
            # Simular movimento: intensidade varia com o tempo
            ...
            for i, (zona, fa, fb) in enumerate(zip(zonas, fA_anim, fB_anim)):
                frame_data.append(dict(
                    type='rect', x0=i*33.33, y0=50, x1=(i+1)*33.33, y1=100,
                    fillcolor=f"rgba(240,192,64,{fa})"
                ))
                frame_data.append(dict(
                    type='rect', x0=i*33.33, y0=0, x1=(i+1)*33.33, y1=50,
                    fillcolor=f"rgba(74,144,217,{fb})"
                ))
        ...
        # Campo
        fig_field.add_shape(type="rect", x0=0, y0=0, x1=100, y1=100,
                           fillcolor="#1B4D1B", line=dict(color="white", width=2))
        fig_field.add_shape(type="line", x0=50, y0=0, x1=50, y1=100,
                           line=dict(color="white", width=2))
        fig_field.add_shape(type="circle", x0=35, y0=35, x1=65, y1=65,
                           line=dict(color="white", width=2))
        
        # Zonas iniciais
        for i, (zona, fa, fb) in enumerate(zip(['Defesa','Meio','Ataque'], fA, fB)):
            fig_field.add_shape(type="rect", x0=i*33.33, y0=50, x1=(i+1)*33.33, y1=100,
                               fillcolor=f"rgba(240,192,64,{fa})", line_width=0)
            fig_field.add_shape(type="rect", x0=i*33.33, y0=0, x1=(i+1)*33.33, y1=50,
                               fillcolor=f"rgba(74,144,217,{fb})", line_width=0)
        
        # Anotações
        for i, (zona, fa, fb) in enumerate(zip(['Defesa','Meio','Ataque'], fA, fB)):
            fig_field.add_annotation(x=i*33.33+16, y=75, text=f"{zona}<br>{fa*100:.0f}%",
                                    showarrow=False, font=dict(color="white", size=11))
            fig_field.add_annotation(x=i*33.33+16, y=25, text=f"{zona}<br>{fb*100:.0f}%",
                                    showarrow=False, font=dict(color="white", size=11))
        
        fig_field.add_annotation(x=15, y=110, text=f"🏠 {nome_casa}", showarrow=False,
                                font=dict(color="#F0C040", size=14))
        fig_field.add_annotation(x=85, y=110, text=f"✈️ {nome_fora}", showarrow=False,
                                font=dict(color="#4a90d9", size=14))
        
        fig_field.frames = frames
        fig_field.update_xaxes(visible=False, range=[0,100])
        fig_field.update_yaxes(visible=False, range=[-10,120])
        ...
        fig_field.update_layout(
            ...
            updatemenus=[dict(type='buttons', showactive=False,
                             buttons=[dict(label='▶️ Iniciar', method='animate',
        ...
        st.markdown("""
        <div style="font-size:13px;color:#B0B8C0;text-align:center;">
            🔸 Dourado = Casa | 🔵 Azul = Visitante<br>
            Clique em ▶️ para ver o fluxo de jogo animado
        </div>
        """,unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

    # ----- ABA 5: Cenários (EXPANDIDA COM TABELA) -----
    with tabs[4]:
        ...
        # Cards de cenários com animação
        for i,(tit,prob,just) in enumerate(cenarios):
            cor_barra = '#F0C040' if prob > 0.50 else '#4A90D9' if prob > 0.30 else '#B0B8C0'
            st.markdown(f"""
            <div style="background:rgba(255,255,255,0.02);border-radius:10px;padding:14px 16px;margin:8px 0;
                        border-left:4px solid {cor_barra};transition:all 0.3s ease;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <span style="font-weight:700;color:#E0E0E0;font-size:15px;">{i+1}. {tit}</span>
                    <span style="font-size:22px;font-weight:900;color:{cor_barra};">{prob:.1%}</span>
                </div>
                <div style="font-size:12px;color:#B0B8C0;margin-top:6px;">{just}</div>
                <div style="background:rgba(255,255,255,0.03);border-radius:4px;height:4px;margin-top:8px;">
                    <div style="width:{prob*100}%;height:4px;background:{cor_barra};border-radius:4px;transition:width 1s ease;"></div>
                </div>
            </div>
            """,unsafe_allow_html=True)
        ...
        with col_res1:
            st.metric(f"Vitória {nome_casa}", f"{p_A:.1%}", delta=f"EC: {EC_A:.1f}")
        with col_res2:
            st.metric("Empate", f"{p_emp:.1%}")
        with col_res3:
            st.metric(f"Vitória {nome_fora}", f"{p_B:.1%}", delta=f"EC: {EC_B:.1f}")
    ...
    with tabs[5]:
        ...
        # Explicação visual do ajuste
        st.markdown(f"""
        <div style="text-align:center;margin:16px 0;">
            <span style="font-size:14px;color:#B0B8C0;">Fator de Ajuste (diferença dos ECs):</span>
            <span style="font-size:28px;font-weight:900;color:#F0C040;margin-left:8px;">{fator_ajuste:+.2f}</span>
        </div>
        """,unsafe_allow_html=True)
        ...
        with col_orig:
            st.markdown(f"""
            <div style="background:rgba(255,255,255,0.02);border-radius:10px;padding:18px;text-align:center;">
                <div style="font-size:13px;color:#B0B8C0;text-transform:uppercase;">📊 Lambdas Originais</div>
                <div style="font-size:32px;font-weight:900;color:#B0B8C0;">🏠 {lambda_casa_orig:.2f}</div>
                <div style="font-size:32px;font-weight:900;color:#B0B8C0;">✈️ {lambda_fora_orig:.2f}</div>
                <div style="font-size:11px;color:#5A6070;margin-top:4px;">Média simples: (GM + GS adv) / 2</div>
            </div>
            """,unsafe_allow_html=True)
        with col_adj:
            st.markdown(f"""
            <div style="background:rgba(240,192,64,0.03);border:1px solid rgba(240,192,64,0.2);border-radius:10px;padding:18px;text-align:center;">
                <div style="font-size:13px;color:#F0C040;text-transform:uppercase;">✨ Lambdas Ajustados</div>
                <div style="font-size:32px;font-weight:900;color:#F0C040;">🏠 {lambda_casa_adj:.2f}</div>
                <div style="font-size:32px;font-weight:900;color:#F0C040;">✈️ {lambda_fora_adj:.2f}</div>
                <div style="font-size:11px;color:#5A6070;margin-top:4px;">Ajustado pelo EngramScore (fator ×0.5)</div>
            </div>
            """,unsafe_allow_html=True)
        ...
        <div class="info-card" style="margin-top:16px;">
            <strong>📈 Impacto do Ajuste:</strong><br>
            🏠 {nome_casa}: expectativa de gols <strong>{'aumentou' if impacto_casa > 0 else 'diminuiu'} {abs(impacto_casa):.1f}%</strong><br>
            ✈️ {nome_fora}: expectativa de gols <strong>{'aumentou' if impacto_fora > 0 else 'diminuiu'} {abs(impacto_fora):.1f}%</strong><br><br>
            <small>Time com EC maior tem seu ataque favorecido e defesa reforçada.</small>
        </div>
        """,unsafe_allow_html=True)
        
        # Gráfico comparativo
        df_lambdas = pd.DataFrame({
            'Métrica': ['λ Casa Original', 'λ Casa Ajustado', 'λ Fora Original', 'λ Fora Ajustado'],
            'Valor': [lambda_casa_orig, lambda_casa_adj, lambda_fora_orig, lambda_fora_adj],
            'Tipo': ['Original', 'Ajustado', 'Original', 'Ajustado'],
            'Time': [nome_casa, nome_casa, nome_fora, nome_fora]
        })
        fig_lambdas = px.bar(df_lambdas, x='Métrica', y='Valor', color='Tipo', barmode='group',
                             color_discrete_map={'Original':'#B0B8C0','Ajustado':'#F0C040'},
                             title="Comparação dos Lambdas")
    ...
    with tabs[6]:
        ...
        # Cards 1X2 com destaque visual
        col_p1,col_p2,col_p3=st.columns(3)
        with col_p1:
            borda = '#00E676' if p_A >= p_emp and p_A >= p_B else '#252B38'
            st.markdown(f"""
            <div class="prob-box" style="border:2px solid {borda};">
                ...
                <div style="font-size:11px;color:#B0B8C0;margin-top:4px;">EC: {EC_A:.1f}</div>
            </div>
            """,unsafe_allow_html=True)
        with col_p2:
            ...
            <div class="prob-box" style="border:2px solid {borda};">
                ...
                <div style="font-size:11px;color:#B0B8C0;margin-top:4px;">Dif. EC: {diff_ec:.1f}</div>
            </div>
            """,unsafe_allow_html=True)
        with col_p3:
            ...
            <div class="prob-box" style="border:2px solid {borda};">
                ...
                <div style="font-size:11px;color:#B0B8C0;margin-top:4px;">EC: {EC_B:.1f}</div>
            </div>
            """,unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

        # Gols totais com gráfico visual
        ...
        df_gols = pd.DataFrame({
            'Mercado': ['Over 0.5', 'Over 1.5', 'Over 2.5', 'Over 3.5', 'BTTS Sim', 'Gol 1ºT'],
            'Probabilidade': [1.0, over15_adj, over25_adj, over35_adj, btts_adj, prob_gol_ht_adj]
        })
        fig_gols = px.bar(df_gols, x='Mercado', y='Probabilidade', text_auto='.1%',
                          color='Probabilidade', color_continuous_scale=['#4A90D9','#F0C040'],
                          title="Probabilidades de Gols")
        fig_gols.update_traces(textposition='outside', textfont=dict(color='white', size=13))
        fig_gols.update_layout(template='plotly_dark',paper_bgcolor='rgba(0,0,0,0)',
        ...
        for mercado,prob in probs_modelo.items():
            ...
            if odd_real and odd_real>1.0:
                ...
                indicacao="💚 VALOR" if ev>5 else "🟢 Bom" if ev>0 else "🔴 Sem"
                linhas.append((mercado,f"{prob:.1%}",f"{odd_mod:.2f}",f"{odd_real:.2f}",f"{ev:+.1f}%",indicacao))
            else:
                linhas.append((mercado,f"{prob:.1%}",f"{odd_mod:.2f}","-","-","⚪"))
    ...
    with tabs[7]:
        ...
        if destaques:
            for nome,prob,cat in destaques:
                ...
                <div style="background:rgba(240,192,64,0.08);border:1px solid rgba(240,192,64,0.3);border-radius:10px;padding:14px;margin:6px 0;display:flex;justify-content:space-between;align-items:center;">
                    <span style="color:#F0C040;font-weight:700;">{icone} {nome}</span>
        ...
        # Diferenciais táticos
        st.markdown("---")
        st.markdown("**🎯 Diferenciais Táticos**")
        
        dif_posse = posse_casa - posse_fora
        dif_chutes = fa_casa - fa_fora
        dif_desarme = des_casa - des_fora
        ...
        # Diferenciais WhoScored (se disponíveis)
    ...
    # ----- ABA 9: Análise Descritiva (EXPANDIDA COM RATING) -----
    with tabs[8]:
        ...
        st.markdown(f"""<div class="info-card">
            <strong>Momento Atual:</strong> {nome_casa} {ma_A:.1f} × {nome_fora} {ma_B:.1f}. {'Casa em melhor fase.' if ma_A>ma_B else 'Visitante em melhor momento.' if ma_B>ma_A else 'Momentos semelhantes.'}<br>
            <strong>Força Geral:</strong> {nome_casa} {fg_A:.1f} × {nome_fora} {fg_B:.1f}. {'Casa mais forte.' if fg_A>fg_B else 'Visitante mais forte.' if fg_B>fg_A else 'Força equilibrada.'}<br>
            <strong>Confronto:</strong> {nome_casa} {cpp_A:.1f} × {nome_fora} {cpp_B:.1f}.<br>
        ...
        st.markdown(f"""<div class="info-card">
            <strong>Ataque:</strong> {nome_casa} {atq_A:.1f} × {nome_fora} {atq_B:.1f}.<br>
            <strong>Defesa:</strong> {nome_casa} {def_A:.1f} × {nome_fora} {def_B:.1f}.<br>
            <strong>Meio:</strong> {nome_casa} {mei_A:.1f} × {nome_fora} {mei_B:.1f}.
        </div>""",unsafe_allow_html=True)
        ...
        # Rating WhoScored (se disponível)
        if dados.get('rating_casa', 0) > 0:
            st.markdown("### ⭐ Rating WhoScored")
            st.markdown(f"""<div class="info-card">
                {nome_casa}: <strong>{dados.get('rating_casa', 6.5):.2f}</strong> | 
                {nome_fora}: <strong>{dados.get('rating_fora', 6.5):.2f}</strong>
            </div>""",unsafe_allow_html=True)

        # Expectativa de gols
        st.markdown("### 📊 Expectativa de Gols")
        st.markdown(f"""<div class="info-card">
            Total esperado: <strong>{lambda_casa_adj+lambda_fora_adj:.2f}</strong> gols.<br>
            Over 1.5: <strong>{over15_adj:.1%}</strong> | Over 2.5: <strong>{over25_adj:.1%}</strong> | Over 3.5: <strong>{over35_adj:.1%}</strong><br>
            BTTS: <strong>{btts_adj:.1%}</strong> | Gol 1ºT: <strong>{prob_gol_ht_adj:.1%}</strong>
        </div>""",unsafe_allow_html=True)
